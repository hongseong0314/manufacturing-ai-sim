...
- Treat `src/mes/harness.py` as the current MES development harness. It must
  evolve toward the two-pass layered architecture described here.
...
- Harness: creates a current `L4 -> L3 -> L1 -> L2` recommendation chain.
...
- True bottom-up candidate portfolio generation.
- L3/L4 selection over candidate portfolios.
- Rule validation that checks upper-layer selection consistency against L1/L2.
